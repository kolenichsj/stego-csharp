...
namespace Hips
{
    ...
    public static class HipsXLSX
    {
        ...
        {
            hipsNamespace = string.IsNullOrEmpty(hipsNamespace) ? HipsXLSX.HipsNamespace : hipsNamespace;

            using var xlDoc = SpreadsheetDocument.Open(srcDocPath, true);
            var mainWBPart = xlDoc.WorkbookPart ?? xlDoc.AddWorkbookPart();
            var newSheetPart = xlDoc.AddNewPart<WorksheetPart>();
            newSheetPart.Worksheet = GenerateWkSheetXML(hipsNamespace, new StringReader(newSheetPart.Worksheet.OuterXml), covertText);
        }
        ...
        public static void InsertText(string srcDocPath, string dstDocPath, string covertText, string hipsNamespace = "")
        {
            ...
            docmain.Document = GenerateWkSheetXML(hipsNamespace, docPartReader, covertText);
        }
        ...
        private static ss.Worksheet GenerateWkSheetXML(string hipsNamespace, TextReader wkSheetText, string covertText)
        {
